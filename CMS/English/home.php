			...
			<div class="dropdown" style="cursor:cell">
				...
				<div id="dropUser" class="d-flex align-items-center" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					...
					<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropUser">
						<a class="dropdown-item active" href="/home/<?php echo $myrow['username']; ?>"><i class="fas fa-user text-muted mr-2"></i>Visit my Profile</a>
			...
			<div class="col-md-8 offset-md-4 pl-md-3">
				<ul class="nav nav-pills" id="tabs" role="tablist">
				    <li class="nav-item active">
						<a class="nav-link active" id="infos-tab" data-toggle="tab" href="#infos" role="tab" aria-controls="infos" aria-selected="true">Curiosities</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" id="quartos-tab" data-toggle="tab" href="#quartos" role="tab" aria-controls="quartos" aria-selected="true">Rooms</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" id="emblemas-tab" data-toggle="tab" href="#emblemas" role="tab" aria-controls="emblemas" aria-selected="true">Badges</a>
					</li>
					<li class="nav-item">
...
<?php } ?>
					...
					<hr>
					<div class="mb-1 text-muted"><font color="#FF9030"><strong><?php echo mysql_real_escape_string($usr->credits); ?></strong> Credits</font></div>
					<div class="mb-1 text-muted"><font color="#822273"><strong><?php echo $user_duc['amount']; ?></strong> Duckets</font></div>
					<div class="mb-1 text-muted"><font color="#0aa8ec"><strong><?php echo $user_dia['amount']; ?></strong> Diamonds</font></div>
					<hr>
					<div class="mb-0 text-muted">
					<?php if($usr->online == '1') { 
					        echo '<strong>Are at the hotel right now.</strong>';
					    } elseif($usr->last_online == '0') { 
				            echo 'He never entered the Hotel.';
					    } else {
					        echo 'Last connection was ' .GetLast($usr->last_online). '.';
					    } ?>
					</div>
			...
			<section class="py-2">
				<div class="tab-content" id="tabsContent">
					...
					<div class="tab-pane fade show active" id="infos" role="tabpanel" aria-labelledby="infos-tab">
					    <h3 class="mb-3">Curiosities about <?php echo mysql_real_escape_string($usr->username); ?></h3>
                        <div class="row">
							<div class="card">
								<div class="card-body text-center text-muted">
									<strong><?php echo mysql_real_escape_string($usr->username); ?></strong> Join at <?php echo $Holo['name']; ?> in <strong><?php echo date("d/m",mysql_real_escape_string($usr->account_created)); ?></strong>, <strong><?php echo date("Y",mysql_real_escape_string($usr->account_created)); ?></strong>, already received <strong><?php echo $user_n['respects_received']; ?></strong> Respect, but also given <strong><?php echo $user_n['respects_given']; ?></strong> Respect, we also inform you that <?php echo mysql_real_escape_string($usr->username); ?> have <strong><?php echo $user_n['achievement_score']; ?></strong> Achievement Score Points, and already bought <strong><?php echo mysql_num_rows(mysql_query("SELECT * FROM items WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?></strong> Furnis.
								</div>
							</div>
						</div>
					</div>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> have <?php echo mysql_num_rows(mysql_query("SELECT * FROM rooms WHERE owner_id = '".mysql_real_escape_string($usr->id)."'")) ?> Rooms</h3>
						<div class="row">
...
<?php } ?>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> have <?php echo mysql_num_rows(mysql_query("SELECT * FROM users_badges WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?> Badges</h3>
						<div class="row">
...
<?php } ?>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> have <?php echo mysql_num_rows(mysql_query("SELECT * FROM camera_web WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?> Photos</h3>
						<div class="row">
