    ...
    <div class="col-xl-12" id="editNews">
        <div class="kt-portlet">
                ...
                <div class="kt-portlet__head-label">
                    ...
                    <h3 class="kt-portlet__head-title">Gerenciar banimentos</h3>
                </div>
                </div>
...
if($do == "dele"){
	...
	if(mysql_num_rows($check) > 0){
		...
		mysql_query("INSERT INTO cms_hklogs SET type = 'Banimentos', note = 'Desbaniu um usuário: $key', author_name = '". $myrow['username'] ."', author_id = '". $myrow['id'] ."', author_rank = '". $myrow['rank'] ."', timestamp = '". time() ."'"); 
		echo '<div class="form-group form-group-last"><div class="alert alert-success" role="alert"><div class="alert-text">Parabéns! Você desbaniu alguém com sucesso! Aguarde.</div></div></div><meta http-equiv="refresh" content="2; url=/housekeeping/bans.php">';
	} else {
...
?>
      ...
      <table class="centered">
        ...
        <tr>
            ...
            <th data-field="ban">Usuário:</th>
			<th data-field="day">Banido dia:</th>
            <th data-field="expire">Expira:</th>
            <th data-field="category">Categoria:</th>
			<th data-field="banned">Responsável:</th>
			<th data-field="option">Desbanir:</th>
	    </tr>
        </thead>
...
$user = mysql_fetch_assoc($user = mysql_query("SELECT * FROM users WHERE id = '".$getb['user_id']."'"));
$staff = mysql_fetch_assoc($staff = mysql_query("SELECT * FROM users WHERE id = '".$getb['user_staff_id']."'"));
?>
        <tbody>
          <tr>
            <td><?php echo $getb['id']; ?></td>
            <td><a href="/home/<?php echo $user['username']; ?>" target="_blank"><?php echo $user['username']; ?></font></td>
            <td><?php echo date('d/m/Y', $getb['timestamp']); ?></td>
            <td><?php echo date('d/m/Y', $getb['ban_expire']); ?></td>
			<td><?php echo $getb['type']; ?></td>
			<td><?php echo $staff['username']; ?></td>
			<td><a href="bans.php?do=dele&key=<?php echo $getb['id']; ?>" class="flaticon-delete"> Desbanir</a></td>
          </tr>	  
        </tbody>
<?php } ?>
	  </table>
